...
from sklearn.metrics import root_mean_squared_error, mean_absolute_error, roc_auc_score, accuracy_score
...
class MixtureLinearModel:
    def __init__(self, base_model, verbose=True):
        self.nn_model = base_model
        self.numlayers = None
        self.verbose = verbose
    
    def compute_kmeans_CELL(self, X_train, K, verbose=None, random_seed=None):
        self.K = K
        # sample size
        ...
        extcls = kmeans.fit_predict(X_train)

        # state sequence / semi_cls id / semi_cls size
        self.CELL = np.unique(extcls, return_inverse=True, return_counts=True)
        ...
        if self.verbose:
            print(f"# of CELL:{self.Ktilde} / min size:{self.CELL_size.min()} / avg size:{self.CELL_size.mean():3.1f} / max size:{self.CELL_size.max()} / # of singleton CELL:{np.sum(self.CELL_size==1)}")
    
    # perturbed samples ------------------------------

    def compute_augmented_samp(self, X_train, y_train,
                              eps=0.001, num_noise_samp=100, classification=True, random_seed=None):
        self.eps = eps
                ...
                if len(np.unique(y_local[k])) < 2:
                    ...
                    for i in self.nn_model:
                        if roc_auc_score(y_local[k], i[0].predict(X_local[k])) >= roc_score:
                            roc_score = roc_auc_score(y_local[k], i[0].predict(X_local[k]))
                            clf_inside = i[0]
                    clf.append(clf_inside)
            else:
                for i in self.nn_model:
                    if rmse(y_local[k], i[0].predict(X_local[k])) <= rmse_score:                        
                        rmse_score = rmse(y_local[k], i[0].predict(X_local[k]))
                        clf_inside = i[0]
                clf.append(clf_inside)
#                 print("clf", clf[-1])
            
#         print("Len of clf:", len(clf))
...
#                 print("cluster:", i, "model:", clf[i])
                y_perturb[i] = pd.DataFrame(clf[i].predict(X_perturb[i]))
#                 y_perturb[i] = np.array(self.nn_model.predict(np.reshape(X_perturb[i], ((num_noise_samp,) + self.p_orig))))
