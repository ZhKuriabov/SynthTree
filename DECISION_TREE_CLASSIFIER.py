...
class DistanceDecisionTree:
    ...
    def copy(self):
        # Create a new instance of the DistanceDecisionTree class
        new_instance = DistanceDecisionTree(self.max_depth, self.classification)
        # Copy the tree and final_depth attributes
...
#                         model = LogisticRegression(solver='saga', penalty='l1')
                        model = sm.Logit(y[data_indices], X[data_indices]).fit_regularized(method='l1', alpha=0.1, maxiter=1000)
#                         model.fit(X[data_indices], y[data_indices])
